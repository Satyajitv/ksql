...
The WHERE clause may refer to any column defined for a stream or table, including the two implicit columns `ROWTIME`
and `ROWKEY`.
...
> Tip: If you are interested in selecting past data, you may need to configure KSQL to query the stream from the very
> beginning before running the query:
>
> ```sql
